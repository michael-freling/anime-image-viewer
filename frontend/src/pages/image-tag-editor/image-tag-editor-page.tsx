...
/**
 * Dispatch the pending add/remove mutation against whichever TagService
 * method the backend exposes. We probe for known aliases at runtime because
 * the bindings are generated by wails3 and may carry either the canonical
 * name (`BatchUpdateTagsForFiles`) or a future rename.
 *
 * If no method is available, we warn and resolve — callers still see the
 * toast and navigate back. This mirrors the pattern from
 * `src/pages/tags/tag-mutations.ts`.
 */
async function dispatchTagMutation(
  ...
  const candidate = [
    "BatchUpdateTagsForFiles",
    "UpdateTagsForFiles",
    "BatchUpdateFileTags",
  ].find((name) => typeof svc[name] === "function");

  if (candidate) {
    const fn = svc[candidate] as (
      ids: readonly number[],
      add: readonly number[],
      remove: readonly number[],
    ) => Promise<unknown>;
    await fn(imageIds, addIds, removeIds);
    return;
  }
